...
/* 同一时间只允许一个翻译请求在飞。
   ...
   请求早就发出去、DeepSeek 那边已经在生成、已经在按 token 计费。
   快速连划时前几次的请求纯属白花钱，而且浮层里只会显示最后一次的结果。
...
/* ⚠️ 一处**故意偏离 #18 初稿**的地方，记在这里：
   ...
   但按 #1 的实测结论：**取消发生在请求发出之后，输入 token 已经计费**，
   砍掉的只是输出那一段。所以被取消的请求**确实花了钱** ——
   不计它，等于让这道防线看不见「请求被反复取消」这种烧钱方式，
   而那正是 #19 存在的理由。
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  /* 顺序是刻意的：**先缓存、再额度、最后发请求**。
     ...
     - 缓存命中既不发请求、也不计次数（没花钱就不该算额度，否则反复划同一个词会莫名撞上限）
     - 额度到顶就不发请求（省一次计费），并且给一个明确的 code 让浮层能放「去设置」 */
  getConfig()
    .then(async (cfg) => {
